...
export default function NewPostPage() {
  ...
  const [post, setPost] = useState<any>({
    ...
    isFeatured: false, isSticky: false, scheduledAt: null,
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      setPost((prev: any) => ({
        ...
        ogImage: aiPost.ogImage || "", category: aiPost.category || "",
        tags: aiPost.tags || [], seoScore: aiPost.seoScore || 0,
  ...
  const handleReplaceInline = () => {
    setBubbleMenuOpen(false);
    setImageModalOpen(true); // this opens modal with range, user can click "Replace Image"
  };
  ...
  const persistPost = async (statusOverride: "draft" | "published"): Promise<boolean> => {
    ...
    const payload = ensureDraftFields({ ...post, status: statusOverride }, fallbackId);
    const currentId = savedPostIdRef.current;
    ...
    if (currentId) {
      ...
      if (res.ok) {
        const data = await res.json();
        const newId = data.post?.id ?? fallbackId;
        setSavedPostId(newId);
        savedPostIdRef.current = newId;
        setPost((p: any) => ({ ...p, title: payload.title, slug: payload.slug }));
      }
    }
    return res.ok;
  };
          ...
          <div className="flex-1 overflow-y-auto px-8 py-8 relative" style={{ background: "#050505" }}>
            ...
            <div className="max-w-3xl mx-auto space-y-6">
              <textarea
                ...
                style={{ fontFamily: "var(--font-anton), Anton, sans-serif", borderColor: "rgba(255,255,255,0.06)" }}
              />
              ...
              <div data-color-mode="dark" className="mt-2">
                <MDEditor
                  ...
                  textareaProps={{
                    placeholder: "Start writing your article…\n\nTip: Use formatting shortcuts above, or click 🖼 Image to insert inline images.",
